...
class Parser {
    ...
    parse(event) {
        ...
        if (activitystreams.object && normalized.hashtags
            ...
            activitystreams.object.tag = R.map((tag) => ({ id: tag.text, name: tag.text, type: 'Object' }), normalized.hashtags);
        }
    ...
    normalize(raw) {
        ...
        const extractText = (txt, username, attachments) => {
            ...
            if (!R.isEmpty(attachments)) {
                R.forEach((attachment) => text = text.replace(`${attachment._url}`, ''), attachments);
            }
